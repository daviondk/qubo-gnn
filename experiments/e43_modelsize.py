...
def main():
    ...
    for ft,ei,(mu,S) in TE:
        q=selection_qubo(mu,S,KK,risk_aversion=LAM,return_weight=1-LAM); refs.append((q,tabu_qubo(q,num_reads=80,seed=0)["energy"]))
    print("E43 model-size ablation (optimizer-amortized, K=15)", flush=True)
    out={}
    for (h,L) in [(8,1),(16,2),(32,2),(64,3),(128,3)]:
        torch.manual_seed(0); np.random.seed(0)
        m=Net(4,h=h,L=L).to(DEV); opt=torch.optim.Adam(m.parameters(),lr=1.3e-3); pw=torch.tensor([(N-KK)/KK],device=DEV)
        for ep in range(400):
        ...
        m.eval(); nparams=sum(p.numel() for p in m.parameters()); g=[]
        for (ft,ei,(mu,S)),(q,ref) in zip(TE,refs):
        ...
        out[f"h{h}_L{L}"]={"params":nparams,"gap":float(np.mean(g))}
        print(f"  hidden={h:>3} layers={L} ({nparams:>6} params): gap vs tabu {np.mean(g):.3f}%", flush=True)
    json.dump(out, open(os.path.join(HERE,"results","e43_modelsize.json"),"w"), indent=2); print("saved", flush=True)
