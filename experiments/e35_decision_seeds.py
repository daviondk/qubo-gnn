...
def main():
    ...
    trw = [(torch.tensor(feats(*est(t))[0], device=DEV), knn(feats(*est(t))[1]), torch.tensor(R[t:t+step].sum(0), dtype=torch.float32, device=DEV)) for t in tr_i]
    ...
    print(f"\n=== E35: decision-focused amortized Sharpe {np.mean(shs):.3f} +/- {np.std(shs):.3f} (5 seeds) vs per-instance tabu {ref_sh:.3f} (optimizer) ===", flush=True)
    json.dump({"decision_sharpe_mean": float(np.mean(shs)), "decision_sharpe_std": float(np.std(shs)), "seeds": shs, "perinst_tabu": ref_sh},
              open(os.path.join(HERE, "results", "e35_decision_seeds.json"), "w"), indent=2); print("saved", flush=True)
