...
def main():
    ...
    # train ONE amortized (imitation) at K=15
    from amortized import sel_obj as SOBJ
    TR=[]
    for t in tr_i:
        mu,S=est(t); q=selection_qubo(mu,S,KTRAIN,risk_aversion=LAM,return_weight=1-LAM)
        r=tabu_qubo(q,num_reads=80,seed=0); xi=np.flatnonzero(np.asarray(r["x"])>0.5)
        sel=xi if len(xi)==KTRAIN else np.argsort(-np.asarray(r["x"]))[:KTRAIN]; f,C=feats(mu,S)
        TR.append((torch.tensor(f,device=DEV),knn(C),torch.tensor(np.isin(np.arange(N),sel).astype(np.float32),device=DEV)))
    torch.manual_seed(0); np.random.seed(0)
    m=Net(4).to(DEV); opt=torch.optim.Adam(m.parameters(),lr=1.3e-3); pw=torch.tensor([(N-KTRAIN)/KTRAIN],device=DEV)
    for ep in range(400):
    ...
    m.eval()
    print(f"E41 K-generalization: trained at K={KTRAIN}, eval across K", flush=True)
    # precompute feats per test window
    TE=[(torch.tensor(feats(*est(t))[0],device=DEV), knn(feats(*est(t))[1]), est(t)) for t in te_i]
    out={}
    for K in [8,15,25,35]:
        g=[]
        for ft,ei,(mu,S) in TE:
            q=selection_qubo(mu,S,K,risk_aversion=LAM,return_weight=1-LAM)
            ref=tabu_qubo(q,num_reads=80,seed=0)["energy"]
            with torch.no_grad(): p=m(ft,ei).cpu().numpy()
            from qubo import QUBO as _Q
            xk=np.zeros(N,np.int8); xk[np.argsort(-p)[:K]]=1
            e=q.energy(xk); g.append((e-ref)/abs(ref)*100 if abs(ref)>1e-12 else 0.0)
        out[K]=float(np.mean(g)); print(f"  K={K:>3} (model trained K=15): amortized top-K gap vs tabu {np.mean(g):.3f}%", flush=True)
    json.dump(out, open(os.path.join(HERE,"results","e41_kgen.json"),"w"), indent=2); print("saved", flush=True)
