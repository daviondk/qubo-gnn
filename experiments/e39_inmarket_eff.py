...
def main():
    ...
    W = windows(dow); sp = int(0.55*len(W)); trW, teW = W[:sp], W[sp:]
    ewd = perf_metrics(np.concatenate([rn @ (np.ones(rn.shape[1])/rn.shape[1]) for _,_,rn in teW]))["sharpe"]
    print(f"E39 in-market sample efficiency (DOW): {len(trW)} train avail, {len(teW)} test; EqualWeight {ewd:.3f}", flush=True)
    out={"equalweight":ewd}
    ...
    json.dump(out, open(os.path.join(HERE,"results","e39_inmarket_eff.json"),"w"), indent=2); print("saved", flush=True)
