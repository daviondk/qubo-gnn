...
def main():
    ...
    W = windows(R); sp = int(0.55*len(W)); trW, teW = W[:sp], W[sp:]
    ewd = perf_metrics(np.concatenate([rn @ (np.ones(rn.shape[1])/rn.shape[1]) for _,_,rn in teW]))["sharpe"]
    print(f"E40 in-market sample efficiency (NASDAQ, BROAD): {len(trW)} train avail, {len(teW)} test; EqualWeight {ewd:.3f}", flush=True)
    out={"equalweight":ewd}
    for n in [5,10,20,len(trW)]:
        m=train(to_train(trW[-n:])); bt=backtest(m, teW)
        out[n]=bt["sharpe"]; print(f"  in-market windows={n:>3}: DF Sharpe {bt['sharpe']:.3f} (turn {bt['turn']:.2f})  beats EW: {bt['sharpe']>ewd}", flush=True)
    json.dump(out, open(os.path.join(HERE,"results","e40_nasdaq_inmarket.json"),"w"), indent=2); print("saved", flush=True)
