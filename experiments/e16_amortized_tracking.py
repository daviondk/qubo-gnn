...
def main():
    ...
    print(f"E16 amortized index-tracking: N={N} K={K} {len(tr_i)}tr/{len(te_i)}te; refs...", flush=True)
    t0 = time.time()
    def ref(t):
    ...
    tr = [(torch.tensor(feats(r[2], r[3])[0], device=DEV), knn(feats(r[2], r[3])[1]),
    ...
    print(f"\n=== E16 BEST amortized index-tracking TE gap = {best[0]:.2f}% (ep{best[1]}); per-inst ~{tpi:.2f}s, amortized ms ===", flush=True)
    json.dump({"best_te_gap%": best[0], "per_instance_s": tpi}, open(os.path.join(HERE, "results", "e16_amortized_tracking.json"), "w"), indent=2)
