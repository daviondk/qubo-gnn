...
def main():
    ...
    idx = list(range(lb, len(R), step)); sp = int(0.7 * len(idx)); tr_i, te_i = idx[:sp], idx[sp:][:20]
    def est(t):
        w = R[t - lb:t]; mu = w.mean(0); S = np.cov(w, rowvar=False); return mu, 0.5 * (S + S.T) + 1e-8 * np.eye(N)
    print(f"E21 warm-start N461: N={N} K={K} {len(tr_i)}tr/{len(te_i)}te; training amortized...", flush=True)
    tr = []
    for t in tr_i:
    ...
    json.dump(out, open(os.path.join(HERE, "results", "e21_warmstart_n461.json"), "w"), indent=2)
