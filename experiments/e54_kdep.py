...
def main():
    ...
    print("E54 K-dependence: amortization gap + hardness (greedy gap) at lambda=0.5", flush=True)
    out={}
    for KK in [5,15,30,50]:
        TR=[]
        for t in tr_i:
        ...
        m=Net(4).to(DEV); opt=torch.optim.Adam(m.parameters(),lr=1.3e-3); pw=torch.tensor([(N-KK)/KK],device=DEV)
        for ep in range(400):
        ...
        m.eval(); ag=[]; hg=[]
        for t in te_i:
            mu,S=est(t); q=selection_qubo(mu,S,KK,risk_aversion=LAM,return_weight=1-LAM); ref=tabu_qubo(q,num_reads=120,seed=0)["energy"]
            eg=greedy_selection(mu,S,KK,risk_aversion=LAM,return_weight=1-LAM)["energy"]; hg.append((eg-ref)/abs(ref)*100 if abs(ref)>1e-12 else 0.0)
            f,C=feats(mu,S)
            with torch.no_grad(): p=m(torch.tensor(f,device=DEV),knn(C)).cpu().numpy()
            xk=np.zeros(N,np.int8); xk[np.argsort(-p)[:KK]]=1
            ag.append((q.energy(xk)-ref)/abs(ref)*100 if abs(ref)>1e-12 else 0.0)
        out[KK]={"amort_gap":float(np.mean(ag)),"greedy_gap":float(np.mean(hg))}
        print(f"  K={KK:>3}: amortization gap {np.mean(ag):.3f}% | greedy gap (hardness) {np.mean(hg):.3f}%", flush=True)
    json.dump(out, open(os.path.join(HERE,"results","e54_kdep.json"),"w"), indent=2); print("saved", flush=True)
