...
def main():
    ...
    refs=[(selection_qubo(mu,S,KK,risk_aversion=LAM,return_weight=1-LAM),) for (_,_,(mu,S)) in TE]
    refs=[(q[0], tabu_qubo(q[0],num_reads=80,seed=0)["energy"]) for q in refs]
    print("E44 GNN-vs-MLP-vs-Linear ablation (does graph structure matter?)", flush=True)
    out={}
    for name,Cls in [("GNN-SAGE",GNNNet),("MLP-noGraph",MLPNet),("Linear",LinNet)]:
        ...
        out[name]=float(np.mean(g)); print(f"  {name:<12}: gap vs tabu {np.mean(g):.3f}%", flush=True)
    json.dump(out, open(os.path.join(HERE,"results","e44_gnn_ablation.json"),"w"), indent=2); print("saved", flush=True)
